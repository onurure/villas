    		...
    		<div class="container">
    			...
    			<div class="left-side">
    				...
    				<div id="logo" class="margin-top-10">
    					...
    					<a href="{{ url('/') }}" class="sticky-logo"><img src="{{ asset('images/logo2.png') }}" alt=""></a>
    				</div>
    			...
    			<div class="right-side">
    				...
    				<ul class="header-widget">
                            ...
                            <select id="langswitcher" onchange="DilDegis()">
                                @if(Cache::get('diller'))
                                    @foreach (Cache::get('diller') as $dil)
                                        ...
                                        <option value="{{$dil->lan}}">{{ $dil->language }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </li>
                        ...
                        @else
                            <li class="with-btn"><a href="{{ url('login') }}" class="sign-in button border"><i class="fa fa-user"></i> {{ Ceviri('Üye Girişi')}} / {{ Ceviri('Üye Ol')}}</a></li>
                        @endif
                        <!--
        					<li>
					...
					<ul id="responsive">
                        ...
                        @if(Request::is('services')||Request::is('service/*/*'))
                            @if(Auth::guard('firm')->check())
                                <li><a class="" href="{{ url('user/my-services') }}">{{ Ceviri('Yeni Hizmet İlanı Ver') }}</a></li>
                            @else
                                <li><a class="" href="{{ url('login#tab3') }}">{{ Ceviri('Yeni Hizmet İlanı Ver') }}</a></li>
                            @endif
